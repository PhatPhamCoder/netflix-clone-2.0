import React from "react";

interface MobileMenuProps {
  visible?: boolean;
}

const MobileMenu: React.FC<MobileMenuProps> = ({ visible }) => {
  if (!visible) {
    return null;
  }

  return (
    <div className="bg-black w-56 absolute top-12 left-22 py-5 flex-col border-2 border-gray-800 flex">
      <div className="flex-flex-col gap-4">
        <div className="px-3 text-center hover:underline">Home</div>
        <div className="px-3 text-center hover:underline">TV Shows</div>
        <div className="px-3 text-center hover:underline">Movies</div>
        <div className="px-3 text-center hover:underline">New & Popular</div>
        <div className="px-3 text-center hover:underline">My List</div>
        <div className="px-3 text-center hover:underline">
          Browse By Language
        </div>
      </div>
    </div>
  );
};

export default MobileMenu;
